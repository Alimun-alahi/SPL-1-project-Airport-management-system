...
public class SimulationEngine {
    ...
    private void printReport(LocalDateTime start, LocalDateTime end) {
        ...
        List<Flight> flights = flightManagement.getFlights();
        int delayed = 0, holding = 0, boarding = 0, arriving = 0;
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("departedFlights.txt"))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split(",");
                if (parts.length >= 9 && parts[8].equalsIgnoreCase("DEPARTED")) {
                    departed++;
                }
            }
        } catch (IOException e) {
        ...
        // Count ARRIVED flights from arrivedFlights.txt
        int arrived = 0;
        List<String> arrivedIds = new ArrayList<>();
        try (BufferedReader br = new BufferedReader(new FileReader("arrivedFlights.txt"))) {
            String line;
            while ((line = br.readLine()) != null) {
                String[] parts = line.split(",");
                if (parts.length >= 9 && parts[8].equalsIgnoreCase("ARRIVED")) {
                    arrived++;
                    if (parts.length >= 2) arrivedIds.add(parts[1]);
                }
            }
        } catch (IOException e) {
            // File may not exist
