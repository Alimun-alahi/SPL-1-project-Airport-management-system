...
public class DepartureFlightManager {
    ...
    public void checkAndDepartFlights(LocalDateTime currentTime) {
        ...
        for (Flight f : flightManagement.getFlights()) {
            if (f.getStatus().equalsIgnoreCase("BOARDING")) {
                LocalDateTime deadline = f.getDepartDateTime().plusMinutes(20);
                if (!currentTime.isBefore(deadline)) {
                    ...
                    System.out.println(YELLOW + "Flight " + f.getFlightInstanceId() +
                            " not departed yet. Current time " + currentTime +
                            " is before departure deadline " + deadline + RESET);
                }
            }
        }
